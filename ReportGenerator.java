...
class ReportGenerator {
    static void generateReport(String fileName, List<String[]> data) {
        ...
        String dashedLine = "-----------------------------------------"; // Used to separate the grid into a header,
                                                                         // body, and footer.

        try {
            printWriter = new PrintWriter(fileName);
        } // Attempt to assign {printWriter} with {fileName}'s output stream}
        ...
        for (int row = 0; row < data.size(); row++) { // Iterates through all rows.
            for (int column = 0; column < data.get(row).length; column++) { // Iterates through all columns of a row.
                ...
                printWriter.printf(formatSpecifier, data.get(row)[column]); // Prints the data in the cell at the
                                                                            // row-column intersection.
            }
            printWriter.println(); // A line will separate each row.
            if (row == 0) {
                printWriter.println(dashedLine);
            } // Separates header from body.
        }
        printWriter.println(dashedLine); // Separates body from footer.
        printWriter.print("Generated on: " + getDateAndTime()); // Footer message displaying when the report was
                                                                // generated.
    ...
    static String getDateAndTime() { // Gets the local date and time so that it can be output as a String.
        ...
        return localDateTime.format(dateTimeFormatter); // Returns {localDateTime} formatted with th {dateTimeFormatter}
                                                        // template.
    }
